                ...
                loff_t *f_pos)
{
	...
	*f_pos += count;
	...
	/*/ Criptografa cada uma das letras de retorno ******************************************************************
	...
	int  chave;
	char  __user *buf_bkp;
	buf_bkp = vmalloc((count+1) * sizeof(char *));
	if (buf_bkp==NULL) goto out;
	...
	printk("Anderson - scull_read *** Fim 001\n");
	printk(buf);
	printk("\n");
	...
	for (i=0 ; i<count ; i++)
	{
		letra  = buf[i];
		chave  = key[key_count];
		buf_bkp[i] = letra ^ chave;
	
		key_count++;
		if(key_count == strlen(key))
			key_count = 0;
	}
	printk("Anderson - scull_read *** Fim 002\n");
	printk(buf_bkp);
	printk("\n");
	// ************************************************************************************************************/
                ...
                loff_t *f_pos)
{
	...
	*f_pos += count;
	...
	// Criptografa cada uma das letras da memória ******************************************************************
	char key[21]   = "12345678901234567890";
	int  key_count = 0;
	int  i;
	int  letra;
	...
	//printk("Anderson - scull_write *** Ini 001\n");
	...
	//printk(buf_ok);
	//printk("\n");
	//printk("Anderson - scull_write *** Fim 001\n");
	// ************************************************************************************************************/
...
void scull_cleanup_module(void)
{
	...
	if (scull_devices) {
		for (i = 0; i < scull_nr_devs; i++) {
			...
			printk(KERN_WARNING "*** Anderson_cleanup *** Major: %d, Minor: %d\n", scull_major, scull_minor + i);
		}
		kfree(scull_devices);
	}
...
static void scull_setup_cdev(struct scull_dev *dev, int index)
{
	...
	printk(KERN_WARNING "*** Anderson_setup *** Major: %d, Minor: %d\n", scull_major, scull_minor + index);
 ...
 */
	...
	for (i = 0; i < scull_nr_devs; i++) {
		...
		printk(KERN_WARNING "*** Anderson_init  *** Major: %d, Minor: %d\n", scull_major, scull_minor + i);
	}
